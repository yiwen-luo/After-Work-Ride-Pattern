...
end_date = '2015-04-30'
pick_time_start = '17:00:00'
pick_time_end = '17:59:00'
longi_low = -74.016179
...
for i, row_raw in enumerate(rows):
    if i > 0:
        ...
        if row[1] == row[3] and row[3] in range_date and pick_time_start <= row[2] and row[2] <= pick_time_end:
            if longi_low <= float(row[7]) and float(row[7]) <= longi_high and lati_low <= float(row[8]) and float(row[8]) <= lati_high:
                longi_dropoff.append(float(row[11]))
                lati_dropoff.append(float(row[12]))


print len(longi_dropoff)
print lati_dropoff
