...
## rides_data
The folder contains scripts to extract historical NYC taxi data and visualize drop-off location on Googlemap, yellow_cab_process.py is to get drop-off location with specified pickup time windows, and YellowcabHeatMap.html file to plot the data points of given criteria on the Google Map for further analysis. rides_data_output includes pictures, basically Google Heatmaps, that generated by the `YellowcabHeatMap.html` for data visualization and analysis.
