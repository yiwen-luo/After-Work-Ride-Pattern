...
## rides_data
The folder contains scripts to analyze historical NYC taxi data with specified pickup and drop-off time, and HTML file to plot the data points of given criteria on the Google Map for further analysis.

## rides_data_output
This folder contains pictures, basically Google Heatmaps, that generated by the `YellowcabHeatMap.html` for data visualization and analysis.
...
- `data_process.py`
	- `data_process.py` process data from NYC historical data and fetch the data to the stdout with given format. We are interested in the data within a time period of certain range with center specified by the peak hour, which is calculated by the previous uber_surge `main.py`.
